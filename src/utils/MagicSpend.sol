...
///
/// @dev Includes a modification in `validatePaymasterUserOp` to only allow
///      withdraw requests initiated by the owner (see L150).
...
contract MagicSpend is Ownable, IPaymaster {
    ...
    mapping(uint256 nonce => mapping(address user => bool used))
        internal _nonceUsed;
    ...
    /// @notice Thrown when tx.origin is not the owner. Owner must initiate withdraw requests.
    ///
    /// @param txOrigin The tx.origin.
    /// @param owner    The contract owner.
    error InvalidTxOrigin(address txOrigin, address owner);
        ...
        returns (bytes memory context, uint256 validationData)
    {
        ...
        if (tx.origin != owner()) {
            revert InvalidTxOrigin(tx.origin, owner());
        }
        ...
        bool sigFailed = !isValidWithdrawSignature(
            userOp.sender,
            withdrawRequest
        );
        validationData =
            (sigFailed ? 1 : 0) |
            (uint256(withdrawRequest.expiry) << 160);
    ...
    function postOp(
        ...
        (uint256 maxGasCost, address account) = abi.decode(
            context,
            (uint256, address)
        );
    ...
    function entryPointAddStake(
        uint256 amount,
        uint32 unstakeDelaySeconds
    ...
    function setMaxWithdrawDenominator(
    ...
    function isValidWithdrawSignature(
        ...
        return
            SignatureCheckerLib.isValidSignatureNow(
                owner(),
                getHash(account, withdrawRequest),
    ...
    function _validateRequest(
        ...
        if (
            withdrawRequest.asset == address(0) &&
            withdrawRequest.amount > maxAllowed
        ) {
